...
class TelaHistorico : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Obtendo os dados de histórico através de um Array<String>
        val array = intent.getStringArrayExtra("array")
        //Apresentando os dados na TextView
        //txtHistorico.text = array?.joinToString("\n\n").toString()

        //Obtendo os dados de histórico pelo Parcelable
        val parcelable = intent.getParcelableExtra<Historico>("parcelable")
        //Apresentando as expressões na tela
        if (parcelable != null) {
            txtHistorico.text = parcelable.historic.joinToString("\n\n")
        }
    }
}